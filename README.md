...
З кореневої папки репозиторію виконайте в терміналі PowerShell:

```powershell
pwsh -ExecutionPolicy Bypass -File .\convert_to_utf8withBOM.ps1
```

Або для Windows PowerShell 5.x:

```powershell
powershell -ExecutionPolicy Bypass -File .\convert_to_utf8withBOM.ps1
```

> **Примітка:** Скрипт потрібно запускати саме як файл (параметр `-File`), а не копіювати його вміст у термінал, оскільки він використовує `$PSScriptRoot` для визначення робочого каталогу.
...
Неофіційний Технічний чат для розробників ПРРО тут: <https://t.me/+8dsx7QAL7ww0ZWYy>
...
Технічний чат розробників пРРО